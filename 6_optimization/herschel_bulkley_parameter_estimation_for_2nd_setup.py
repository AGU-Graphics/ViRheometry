...
from_dirs = [args.first_dir, args.second_dir]
...
class Main:
    ...
    def exportXML(self, m_breve):
        ...
        Hcm = str(round(s.H, 2))
        Wcm = str(round(s.W, 2))
        arg_1 = "../data/ref_" + material_name + "_" + Hcm + "_" + Wcm + "_3/"
        if not os.path.exists(arg_1):
            os.mkdir(arg_1)
        arg_2 = "settings.xml"
        arg_3 = "../data/" + material_name + "_3"
        arg_4 = Hcm
        arg_5 = Wcm
        arg_6 = str(self.rho)
    ...
    def optimization_2(self):

        m_breve = self.initial_material
        print("initial mateiral: ")
        ...
        cmaes = CMAES_PWQB_LS(7,10,2.0/3,["MAXITER"],100, "example2.txt", lbounds, ubounds)
        ...
        print("m_breve2 : ", m_breve.eta,", ", m_breve.n,", ", m_breve.sigmaY)
        for s in self.setups:
            print("setup: ", "H=", s.H, "W=", s.W, "weight=", s.weight)
        mechanism = Mechanism3D()
        self.setups = mechanism.searchNewSetup_orthognality_for_third_setup(m_breve, self.setups)
...
def main():
    ...
    xmlDatas = [xmlData1, xmlData2]
    optimizer = Main(xmlDatas)
    optimizer.optimization_2() 
